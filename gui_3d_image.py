...
def Confirm_nocrop_button():
    ...
    stack = (cv.imread(_dir+'/3d'+str(i)+'.png',cv.IMREAD_GRAYSCALE) for i in np.arange(len(os.listdir('Large')-1))
    w,h = (((i.shape[0]) for i in stack),((i.shape[1]) for i in stack))
    # TODO: Currently assumes all images the same size and thus require the same scaling
    if (h[0] > w[0]):
        scalefactor = 512 / h[0]
    else:
        scalefactor = 512 / w[0]
    swidth = int(scalefactor * w[0])
    sheight = int(scalefactor * h[0])
    dsize = (sheight, swidth)
    
    src = (cv2.resize(i, dsize) for i in stack)
...
def make_gui(window):
    ...
    button1 = Button(frame3, text="Proceed without crop", fg="Black", command=Confirm_nocrop_button_ex(True))
    button2 = Button(frame3, text="Exit", bg="Red", command=frame3.quit)
    button3 = Button(frame2, text="Select stack..", bg="gray", command=getstack)
